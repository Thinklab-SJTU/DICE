...
def get_model(model_name, dataset_name, pretrained=False, causal_model_path=None):
    """Retrieve an appropriate architecture."""
    if 'CIFAR' in dataset_name or 'MNIST' in dataset_name:
        if pretrained and model_name != 'causal_v0':
        ...
        elif model_name == 'causal_v0':
            import sys
            sys.path.append("../causal_attack/")
            from util_func import make_model
            from model.CRLv0 import CausalRLNetwork
            backbone = make_model('WideResNet')
            model = CausalRLNetwork(backbone)
            if pretrained:
                assert causal_model_path is not None
                model.load_state_dict(torch.load(causal_model_path))
        else:
