...
from models import *
from madrys import madrys_loss
...
with DupStdoutFileManager(os.path.join(args.load_path, f"eval_log_{args.step}_{args.epsilon}_{args.runs}.log")) as _:
    print(args)
    ...
    for run in range(args.runs):
        ...
        def train(epoch):
            ...
            for batch_idx, (inputs, targets, clean_inputs) in enumerate(trainloader):
                ...
                if args.step >= 5:
                    step_size = (args.epsilon / 5)/ 255.
                else:
                    step_size = (args.epsilon / args.step) / 255
                loss, adv_correct = madrys_loss(model=net,
                                           x_natural=inputs,
                                           y=targets,
                                           optimizer=optimizer,
                                           step_size= step_size,
                                           epsilon=args.epsilon / 255.,
                                           perturb_steps=args.step)
                loss.backward()
                optimizer.step()
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,milestones=[40,80],gamma=0.1,last_epoch=-1)
